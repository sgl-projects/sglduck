...
from sglduck.cta import Aggregation, SglCta, SglCtaAvg
...
def test_agg_col_name_with_no_scale():
    ...
    assert SglCtaAvg().agg_col_name(col_expr, None) == "sglduck.linear.avg.col_1"


def test_agg_col_name_with_scale():
    col_expr = {"column": "col_1", "cta": SglCtaAvg()}
    assert (
        SglCtaAvg().agg_col_name(col_expr, SglScaleLog()) == "sglduck.log.avg.col_1"
    )
