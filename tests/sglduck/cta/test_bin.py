...
from sglduck.cta import SglCta, SglCtaBin
...
def test_add_transformed_column_adds_correct_values_for_linear_scale():
    ...
    np.testing.assert_allclose(result["sglduck.linear.bin.5.col_1"], expected)
...
def test_add_transformed_column_adds_correct_values_for_log_scale():
    ...
    np.testing.assert_allclose(result["sglduck.log.bin.5.col_1"], expected)
