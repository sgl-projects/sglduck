...
from sglduck.cta import Aggregation, SglCta, SglCtaCount
...
def test_agg_col_name_is_sglduck_count_with_no_scale():
    assert SglCtaCount().agg_col_name({}, None) == "sglduck.count"


def test_agg_col_name_is_sglduck_count_with_scale():
    assert SglCtaCount().agg_col_name({}, SglScaleLog()) == "sglduck.count"
